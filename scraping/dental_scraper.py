...
# 정보 입력해서 보험비(+내용) 받아오는 코드 짜기: AIA사이트 (함수이용)
def getAIAData(name, birth, gender):
    ...
    if gender == 1:
        femaleBtn = driver.find_element_by_xpath(
              # click-event가 붙어있는 코드(div class = "btn-gender-select btn-female")의 xpath 가져와야 함
            '//*[@id="calculator-container-form"]/div[1]/div[2]/div/div[1]/div/div[3]/div[1]/div[1]')
        femaleBtn.click() # 여자 버튼 클릭
    else:
    ...
    resultBtn.click()
    driver.implicitly_wait(3) # 시간대기 줘도 됨 
...
# 정보 입력해서 보험비(+내용) 받아오는 코드 짜기: 라이나생명사이트
...
# getAIAData('g', '960607', 1) # AIA 사이트 실행
# getLinaData('g', '960607', 1) # 라이나생명 사이트 실행